...
`schema.sql`** — it won't touch existing tables. Instead, open the SQL
Editor and run [`supabase/migrations/002_recruiter_lightweight_flow.sql`](./supabase/migrations/002_recruiter_lightweight_flow.sql).
It rebuilds `recruiters` and `intro_requests` for the no-signin model and
adds the new `activity_log` table — your `talents` data (and any test
